...
- Eval harness: `POST /eval/{ref_type}/{ref_id}` mark endpoint (form body), `GET /eval` top-10 dashboard with live precision@10, `GET /eval/marks.json` export (`signal_engine/web/`)
- Operator crontab documented with a 09:00 daily health-check entry (`docs/crontab.txt`)
...
- `python-multipart` added to runtime deps for form parsing on the eval route

### Fixed

- Eval mark form now reads `verdict` from the form body; the previous setup 404'd on submit. Redirect target moves from missing `/` to `/eval` so the operator lands back on the page they were marking
...
- Eval harness (mark + dashboard + JSON export) — the operator's improvement loop is now closed
- LLM provider layer with hard monthly budget ledger
...
[Unreleased]: https://github.com/10xdev4u-alt/signal-engine/compare/v0.2.0...HEAD
